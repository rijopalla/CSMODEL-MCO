...
class KMeans(object):
    ...
    def initialize_centroids(self, data):
        ...
        for i in range(1, self.k):
            ...
            for j in range(len(self.centroids)):
                distance = self.get_euclidean_distance(data.loc[:, data.columns!='class'], self.centroids.loc[j])
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        sq_diff = point2.sub(point1).pow(2)
        if isinstance(point1, pd.DataFrame) or isinstance(point2, pd.DataFrame):
    ...
    def group_observations(self, data):
        ...
        for i in range(self.k):
            distance = self.get_euclidean_distance(sliced_data.loc[:, sliced_data.columns!='class'], self.centroids.loc[i])
            distances.insert(len(distances.columns), len(distances.columns), distance)
